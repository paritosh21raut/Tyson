...
byte RMCS2303::WriteSingleRegister(byte slave_id, int address, unsigned int value)
{
    ...
    if (Sprt == 0)
    {
        while (port->available()) port->read();      // Flush old data

        port->print(result);

        unsigned long start = millis();
        while (!port->available() && (millis() - start < 10));
    ...
    if (Sprt == 1)
    {
        while (softport->available()) softport->read();   // Flush old data

        softport->print(result);

        unsigned long start = millis();
        while (!softport->available() && (millis() - start < 10));
